...
// transaction screenshot uploader
...
// router.post('/submit-details',middleware_check_login, async (req, res) => {
...
router.post("/submit-details", async (req, res) => {
  ...
  const email = "abizersusnerwala786@gmail.com";
...
router.get("/main-dashboard", middleware_check_login, async (req, res) => {
  ...
  let totalEBTRewardsDestributed =
    await adminServices.totalEBTRewardsDestributed(total_users_s);
  ...
  if (admin_detail != "" && admin_detail == null) {
    ...
    res.render("admin/front-admin/main-dashboard.ejs", {
      ...
      total_orders_s: total_orders,
      EBT_balance: EBT_bal,
      ether_balance: actual_balance,
      main_rwn_vault: "0",
      total_users_s,
      ebtSold,
...
router.get("/my-profile", middleware_check_login, async (req, res) => {
  ...
  if (id != "" && id != undefined) {
    ...
    if (admin != "" && admin == null) {
      ...
      res.render("admin/front-admin/my-profile.ejs", {
        Name: req.session.user_name,
        name: admin.name,
        email: admin.email,
        mobile: admin.mobile,
...
router.post("/update_profile", middleware_check_login, (req, res) => {
...
router.get(
  ...
  (req, res) => {
    ...
    AdminInfo.findOne()
      .then(
        (succ) => {
          res.render("admin/front-admin/change-password.ejs", {
            Name: req.session.user_name,
            profile_image: req.session.profile_image,
            user_main_id: succ._id,
            err_msg,
            success_msg,
          });
        },
...
router.post(
  ...
  (req, res) => {
    ...
    var old_pass1 = mykey1.updateOne(old_pass, "utf8", "hex");
    ...
    AdminInfo.findOne({ _id: id }, function (err, result) {
      if (err) {
        ...
        if (check_old_pass != old_pass1) {
          ...
          var new_pass1 = mykey2.updateOne(new_pass, "utf8", "hex");
...
// router.get('/logout1', middleware_check_login, (req, res) => {
//   console.log("logout")
//   req.session.destroy(function (err) {
...
router.get("/user-list", middleware_check_login, (req, res) => {
  err_msg = req.flash("err_msg");
  success_msg = req.flash("success_msg");
    ...
    .then(
      async (results) => {
        ...
        for (var j = 0; j < results.length; j++) {
          var allWalets = [];
          var wallets = await Userwallet.find({
            user_id: results[j]._id,
            deleted: "0",
          });
          // console.log(wallets);
        ...
        if (results) {
          ...
          res.render("admin/front-admin/user-list.ejs", {
            err_msg,
            success_msg,
            expressFlash: req.flash(),
...
router.post("/userListByDate", middleware_check_login, (req, res, next) => {
  ...
  Registration.find({
    deleted: "0",
    created_at: { $gte: min + ", 00:00:00 AM", $lte: max + ", 12:59:59 PM" },
  })
    .sort({ _id: -1 })
    .lean()
    .then(
...
router.post("/searchUser", middleware_check_login, (req, res, next) => {
  ...
  Registration.find({
    ...
    .then(
      async (results) => {
        ...
        await Userwallet.find({
          deleted: "0",
          wallet_address: { $regex: ".*" + value + ".*", $options: "i" },
        }).then(async (searchWallet) => {
          ...
          if (searchWallet != "" && searchWallet != []) {
            for (var l = 0; l < searchWallet.length; l++) {
              ...
              await Registration.findOne({ deleted: "0", _id: search_user_id })
                .sort({ _id: -1 })
                .lean()
                .then(async (results1) => {
                  ...
                  var allWalets1 = [];
                  var wallets1 = await Userwallet.find({
                    user_id: results1._id,
                    deleted: "0",
                  });
        ...
        res.render("admin/front-admin/user_search", {
          expressFlash: req.flash(),
          user_details: all_result,
          moment,
          session: req.session,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.get("/deactiveUser", adminServices.deactivateUser);

router.get("/activeUser", adminServices.activateUser);

router.get("/verifyorder", adminServices.VerifyOrder);

router.get("/edit-user", middleware_check_login, (req, res) => {
  err_msg = req.flash("err_msg");
  success_msg = req.flash("success_msg");
  var user_main_id = req.query.id.trim();

  Registration.findOne({ email: user_main_id })
    .then(
      (results) => {
        console.log("570-/edit-User", results);

        if (results) {
          res.render("admin/front-admin/change_user_password", {
            err_msg,
            success_msg,
            details: results,
            Name: req.session.user_name,
      ...
      if (err) {
        ...
        const mailOptions = {
          ...
          text:
            ...
            "Here is the change password link: http://" +
            req.headers.host +
            "/Profile" +
            "\n\n" +
            "Thanks and Regards," +
...
router.get("/send-token", middleware_check_login, async (req, res) => {
  ...
  if (admin_detail != "" && admin_detail == null) {
    ...
    if (id && id != "" && id != undefined) {
      ...
      await OrderDetails.findOne({ _id: id })
        .then(async (result) => {
          console.log(`660-result`, result);
          var reciever_address = result.rwn_wallet_address;
          var amount = result.rwn_count;
          var user_id = result.user_id;
          await RefCode.findOne({ user_id: user_id, status: "Not used" }).then(
            async (ref) => {
              console.log("665-refResult", ref);
              if (ref != null) {
                var refID = ref._id;
                var reg_code = ref.reg_ref_code;
                await Registration.findOne({ ref_code: reg_code })
                  .then(async (bonusUser) => {
                    var bonusUserId = bonusUser._id;
                    await Importwallet.findOne({
                      user_id: bonusUserId,
                      login_status: "login",
                    })
                      .then(async (loginwallet) => {
                        if (
                          loginwallet != "" &&
                          loginwallet != null &&
                          ...
                          await Userwallet.findOne({
                            ...
                            .then(async (bonususerAdd) => {
                              ...
                              var bonusWalletAddress =
                                bonususerAdd.wallet_address;
                              res.render("admin/front-admin/send-token.ejs", {
                                sender_address,
                                id,
                                Name: req.session.user_name,
                                profile_image: req.session.profile_image,
...
router.post("/submit-token", middleware_check_login, async (req, res, next) => {
  ...
  AdminInfo.findOne({ email: email }).then(async (succ) => {
    if (succ.user_type == "manager") {
      var entered_passphrese1 = crypto
        .createHash("sha256")
        .updateOne(entered_passphrese)
        .digest("base64");
              ...
              if (element1.address == sender_address) {
                ...
                await request(options6, async function (error, response, body) {
                  ...
                  if (!error && response.statusCode == 200) {
                    var csv = body;
                    console.log(csv);
                    var c = web3js.eth.accounts.decrypt(
                      csv,
                      entered_passphrese
                    );
                    ...
                    var options = {
                      ...
                      body: JSON.stringify({
                        ...
                        params: [sender_address, entered_passphrese],
                        id: 1,
                      }),
                    };
                      ...
                      if (!error && response.statusCode == 200) {
                        ...
                        request(options, function (error5, response5, body5) {
                          if (!error5 && response5.statusCode == 200) {
                            ...
                            var tokenContract = new web3js.eth.Contract(
                              user1,
                              "0xb1b370178BfCe52Db662fbEF0AF0EE7DDB2f5719"
                            );
                            var count;
                            var array_donation = [];
                            ...
                            tokenContract.methods
                              ...
                              .then(function (result) {
                                ...
                                if (rown_bal >= get_amount) {
                                  web3js.eth
                                    .getTransactionCount(sender_address)
                                    .then((ex) => {
                                      ...
                                      var rawTransaction = {
                                        ...
                                        data: tokenContract.methods
                                          .transferAndDonateTo(
                                            ...
                                            array_donation,
                                            "0x5Cb3c2f2fD2502723841728C9771bB4E41A156eE"
                                          )
                                          .encodeABI(),
                                      ...
                                      web3js.eth.sendSignedTransaction(
                                        ...
                                        (err, hash) => {
                                          ...
                                          if (
                                            ...
                                            var indiaTime = new Date(indiaTime);
                                            var created_at =
                                              indiaTime.toLocaleString();

                                            Tokendetails.count(function (
                                              err,
                                              respcount
                                            ) {
                                              var count_val =
                                                parseFloat(respcount) +
                                                parseFloat(1);

                                              var TokendetailsData =
                                                new Tokendetails({
                                                  auto: count_val,
                                              ...
                                              TokendetailsData.save(function (
                                                err,
                                                doc
                                              ) {
                                                if (err) {
                                                  console.log(
                                                    "token data is not save."
                                                  );
                                                } else {
                                                  /******Store in session*******/
                                                  var err_msg = "";
                                                  ...
                                                  if (
                                                    ...
                                                    setTimeout(function () {
                                                      res.redirect(
                                                        `/referral?bonusWalletAddress=${bonusWalletAddress}&result=${result}&bonusAmount=${bonusAmount}&sender_address=${sender_address}&user_id=${user_id}&refID=${refID}&orderId=${orderId}`
                                                      );
                                                    }, 15000);
                                                  } else {
                                                    if (orderId) {
                                            ...
                                            req.flash(
                                              "fail",
                                              "Insufficient funds In Your account.1"
                                            );
                                  ...
                                  req.flash(
                                    "fail",
                                    "Insufficient funds In Your account.2"
                                  );
                                  res.redirect("send-token");
                                }
                              });
      ...
      sender_private_key =
        "0de3838ca99bd85255bc630733f7d72484508cc3a8cd9c03a59d6d97aa9bf83b";
      var privateKey = Buffer.from(sender_private_key, "hex");
      if (entered_passphrese1 == matching_pass) {
        console.log("private key", privateKey);
        var options = {
                          ...
                          web3js.eth.sendSignedTransaction(
                            ...
                            (err, hash) => {
                              ...
                              if (
                                ...
                                var indiaTime = new Date().toLocaleString(
                                  "en-US",
                                  { timeZone: "Europe/London" }
                                );
                                var indiaTime = new Date(indiaTime);
                                ...
                                Tokendetails.count(function (err, respcount) {
                                  var count_val =
                                    parseFloat(respcount) + parseFloat(1);
...
router.get("/referral", middleware_check_login, (req, res, next) => {
  var array_donation = [];
  ...
  var sender_private_key =
    "ca45278ef6a080b00e4b38f998362f622fa8a336b9d75f20eb0609e53cfc1d16";
  var privateKey = Buffer.from(sender_private_key, "hex");
  ...
  var tokenContract = new web3js.eth.Contract(
    user1,
    "0x0296b77A92Ee270b8a0b157Cc744E9f102af6C51"
  );
  // if(bonusWalletAddress!=""){
  ...
  if (rown_bal1 >= bonusAmount) {
    web3js.eth.getTransactionCount(sender_address).then(function (w) {
      ...
      web3js.eth.sendSignedTransaction(
        "0x" + transactions.serialize().toString("hex"),
        (errr, hashs) => {
                  ...
                  OrderDetails.updateOne(
                    ...
                    function (err, result) {
                      if (err) {
                        ...
                        Registration.findOne({ _id: user_id }).then(
                          (bonusUser) => {
                            Tokendetails.count(function (err, respcounts) {
                              var count_vals =
                                parseFloat(respcounts) + parseFloat(1);
                              var indiaTime = new Date().toLocaleString(
                                "en-US",
                                { timeZone: "Europe/London" }
                              );
...
/*****************Rowan token history****************/
router.get("/artw-token-history", middleware_check_login, (req, res) => {
  AdminInfo.findOne({ email: req.session.re_usr_email }).then((admin_info) => {
    if (admin_info.user_type == "admin") {
      Tokendetails.find({ deleted: "0" })
        .sort([["auto", -1]])
        .then(
          (success_response) => {
            res.render("admin/front-admin/artw_token_history", {
              Name: req.session.user_name,
              profile_image: req.session.profile_image,
              ether_data: success_response,
            });
          },
          (err) => {
...
router.get("/update-token", middleware_check_login, async (req, res) => {
  err_msg = req.flash("err_msg");
  success_msg = req.flash("success_msg");
  await Tokensettings.findOne()
    .then(async (result) => {
...
router.post("/update-token-details", middleware_check_login, (req, res) => {
...
router.get("/BTC-OrderList", middleware_check_login, (req, res) => {
  OrderDetails.find({ payment_type: "BTC" })
    ...
    .then(
      (results) => {
        console.log(results);
        if (results) {
          res.render("admin/front-admin/BTC-order-list", {
            expressFlash: req.flash(),
            user_details: results,
            moment,
            Name: req.session.user_name,
...
router.get("/ETH-OrderList", middleware_check_login, (req, res) => {
  OrderDetails.find({ payment_type: "ETH" })
    .populate({ path: "user_id" })
    .sort({ _id: -1 })
    .then(
      (results) => {
        if (results) {
          res.render("admin/front-admin/ETH-order-list", {
            expressFlash: req.flash(),
            user_details: results,
            moment,
            Name: req.session.user_name,
...
router.get("/XRP-OrderList", middleware_check_login, (req, res) => {
  OrderDetails.find({ payment_type: "XRP" })
    .populate({ path: "user_id" })
    .sort({ _id: -1 })
    .then(
      (results) => {
        console.log(results);
        if (results) {
          res.render("admin/front-admin/XRP-order-list", {
            expressFlash: req.flash(),
            user_details: results,
            moment,
            Name: req.session.user_name,
...
router.get("/LTC-OrderList", middleware_check_login, (req, res) => {
  OrderDetails.find({ payment_type: "LTC" })
    .populate({ path: "user_id" })
    .sort({ _id: -1 })
    .then(
      (results) => {
        console.log(results);
        if (results) {
          res.render("admin/front-admin/LTC-order-list", {
            expressFlash: req.flash(),
            user_details: results,
            moment,
            Name: req.session.user_name,
...
router.get("/DASH-OrderList", middleware_check_login, (req, res) => {
  OrderDetails.find({ payment_type: "DASH" })
    .populate({ path: "user_id" })
    .sort({ _id: -1 })
    .then(
      (results) => {
        console.log(results);
        if (results) {
          res.render("admin/front-admin/DASH-order-list", {
            expressFlash: req.flash(),
            user_details: results,
            moment,
            Name: req.session.user_name,
...
router.get("/cancelBTC", middleware_check_login, (req, res, next) => {
  var id = req.query.id;
  OrderDetails.updateOne(
    { _id: id },
    {
...
router.get("/cancelETH", middleware_check_login, (req, res, next) => {
  var id = req.query.id;
  OrderDetails.updateOne(
    { _id: id },
    {
...
router.get("/cancelXRP", middleware_check_login, (req, res, next) => {
  var id = req.query.id;
  OrderDetails.updateOne(
    { _id: id },
    {
...
router.get("/cancelLTC", middleware_check_login, (req, res, next) => {
  var id = req.query.id;
  OrderDetails.update(
    { _id: id },
    {
...
router.get("/cancelDASH", middleware_check_login, (req, res, next) => {
  var id = req.query.id;
  OrderDetails.update(
    { _id: id },
    {
...
router.get("/stakes-list", middleware_check_login, async (req, res) => {
  err_msg = req.flash("err_msg");
  success_msg = req.flash("success_msg");

  var stakes = [];
  await MainStake.find({})
    .sort({ _id: -1 })
    .then(async (success_response) => {
      if (success_response) {
        for (var i = 0; i < success_response.length; i++) {
          await Registration.findOne({ _id: success_response[i].user_id }).then(
            async (user) => {
              await Stake.find({ m_stake_id: success_response[i]._id })
                .sort({ _id: -1 })
                .lean()
                .then((vault_list) => {
        ...
        res.render("admin/front-admin/stake-list", {
          err_msg,
          success_msg,
          expressFlash: req.flash(),
          stakes: stakes,
...
router.get("/set-stake", middleware_check_login, (req, res) => {
  err_msg = req.flash("err_msg");
  success_msg = req.flash("success_msg");
  ...
  StakeRate.findOne()
    .then(
      (SetVaultData) => {
        ...
        res.render("admin/front-admin/set-stake.ejs", {
          err_msg,
          success_msg,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.post("/update_set_stake", middleware_check_login, (req, res) => {
...
router.get("/home-banner-details", middleware_check_login, (req, res) => {
  err_msg = req.flash("err_msg");
  success_msg = req.flash("success_msg");
  ...
  BannerInfo.find({ deleted: "0" })
    .then(
      (BannerData) => {
        console.log(BannerData);
        res.render("admin/front-admin/home-banner-details.ejs", {
          err_msg,
          success_msg,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.get("/add-new-banner", middleware_check_login, (req, res) => {
  AdminInfo.findOne()
    .then(
      (succ) => {
        res.render("admin/front-admin/add-new-banner.ejs", {
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
          user_main_id: succ._id,
          session: req.session,
...
router.post("/add_banner", middleware_check_login, (req, res) => {
  const form = formidable({ multiples: true });
  form.parse(req, (err, fields, files) => {
    var imageFile =
      typeof files.banner_image !== "undefined" ? files.banner_image.name : "";
    if (imageFile != "") {
...
router.get("/edit-banner", middleware_check_login, (req, res) => {
  var banner_id = req.query.id;
  console.log(banner_id);
  BannerInfo.findOne({ _id: banner_id, deleted: "0" })
    .then(
      (BannerDetail) => {
        console.log("BannerDetail", BannerDetail);

        res.render("admin/front-admin/edit-banner.ejs", {
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
          session: req.session,
          expressFlash: req.flash(),
...
router.post("/update_banner", middleware_check_login, (req, res) => {
  const form = formidable({ multiples: true });
  form.parse(req, (err, fields, files) => {
    if (files != null && files != undefined && files != "") {
      var imageFile =
        typeof files.banner_image !== "undefined"
          ? files.banner_image.name
          : "";
    } else {
      var imageFile = "";
    }
...
router.get("/delete-banner", middleware_check_login, (req, res) => {
  var current_time = Date.now();

  var banner_id = req.query.id;

  BannerInfo.findByIdAndUpdate(
    banner_id,
    { $set: { deleted: "1", updated_at: current_time } },
    { new: true }
  )
    .then(
      (success) => {
...
router.get("/key-features", middleware_check_login, (req, res) => {
  KeyFeaturesInfo.find({ deleted: "0" })
    .then(
      (keyFeatures) => {
        res.render("admin/front-admin/key-features", {
          err_msg,
          success_msg,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.get("/add-new-key-features", middleware_check_login, (req, res) => {
  res.render("admin/front-admin/add-new-key-features", {
    err_msg,
    success_msg,
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
...
router.post("/add-new-key-features", middleware_check_login, (req, res) => {
  const form = formidable({ multiples: true });
  form.parse(req, (err, fields, files) => {
    var imageFile =
      typeof files.banner_image !== "undefined" ? files.banner_image.name : "";
...
router.get("/edit-key-features", middleware_check_login, (req, res) => {
  var feature_id = req.query.id;
  console.log(feature_id);
  KeyFeaturesInfo.findOne({ _id: feature_id, deleted: "0" })
    .then(
      (featureDetail) => {
        console.log("featureDetail", featureDetail);

        res.render("admin/front-admin/edit-key-feature.ejs", {
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
          session: req.session,
          expressFlash: req.flash(),
...
router.post("/update-key-features", middleware_check_login, (req, res) => {
...
router.get("/delete-key-feature", middleware_check_login, (req, res) => {
  id = req.query.id;
  KeyFeaturesInfo.findOne({ _id: id, deleted: "0" })
    .then(
      async (feature) => {
        feature.deleted = "1";
        await feature.save();
...
router.get("/problem", middleware_check_login, (req, res) => {
  problemInfo
    .find({ deleted: "0" })
    .then(
      (problemDetails) => {
        //console.log(problemDetails)
        res.render("admin/front-admin/problem", {
          err_msg,
          success_msg,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.get("/add-new-problem", middleware_check_login, (req, res) => {
  res.render("admin/front-admin/add-new-problem", {
    err_msg,
    success_msg,
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
...
router.post("/add-new-problem", middleware_check_login, (req, res) => {
  console.log(req.body);
  const form = formidable({ multiples: true });
  form.parse(req, (err, fields, files) => {
    var imageFile =
      typeof files.problemImage !== "undefined" ? files.problemImage.name : "";
    if (imageFile != "") {
...
router.get("/edit-problem", middleware_check_login, (req, res) => {
  console.log("In edit problem");
  id = req.query.id;
  problemInfo
    .findOne({ _id: id, deleted: "0" })
    .then(
      (problem) => {
        res.render("admin/front-admin/edit-problem", {
          err_msg,
          success_msg,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.post("/edit-problem", middleware_check_login, (req, res) => {
  console.log("In edit problem");
  let problem_image;
  const form = formidable({ multiples: true });
  form.parse(req, (err, fields, files) => {
    if (files != null && files != undefined && files != "") {
      var imageFile =
        typeof files.problem_image !== "undefined"
          ? files.problem_image.name
          : "";
    } else {
      var imageFile = "";
    }
...
router.get("/delete-problem", middleware_check_login, (req, res) => {
...
//////////*****************solutions********************////////
router.get("/solution", middleware_check_login, (req, res) => {
  solutionInfo
    .find({ deleted: "0" })
    .then(
      (solutionsDetails) => {
        //console.log(solutionsDetails)
        res.render("admin/front-admin/solution", {
          err_msg,
          success_msg,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.get("/add-new-solution", middleware_check_login, (req, res) => {
  res.render("admin/front-admin/add-new-solution", {
    err_msg,
    success_msg,
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
...
router.post("/add-new-solution", middleware_check_login, (req, res) => {
  console.log(req.body);
  ...
  form.parse(req, (err, fields, files) => {
    var imageFile =
      typeof files.solutionImage !== "undefined"
        ? files.solutionImage.name
        : "";
    if (imageFile != "") {
...
router.get("/edit-solution", middleware_check_login, (req, res) => {
  id = req.query.id;
  ...
  solutionInfo
    ...
    .then(
      (solution) => {
        res.render("admin/front-admin/edit-solution", {
          err_msg,
          success_msg,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.post("/edit-solution", middleware_check_login, (req, res) => {
  console.log(req.body);
  date = getFormattedDate();
  console.log(date);
  const form = formidable({ multiples: true });
...
router.get("/delete-solution", middleware_check_login, (req, res) => {
...
////////////************* token-allocation ******************//////////
router.get("/token-allocation", middleware_check_login, (req, res) => {
  tokenAllocation
    .find({ deleted: "0" })
    .then(
      (tokenAllocationDetails) => {
        //console.log(solutionsDetails)
        res.render("admin/front-admin/token-allocation", {
          err_msg,
          success_msg,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.get("/add-new-token-allocation", middleware_check_login, (req, res) => {
  res.render("admin/front-admin/add-new-token-allocation", {
    err_msg,
    success_msg,
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
...
router.post("/add-new-token-allocation", middleware_check_login, (req, res) => {
  console.log(req.body);
  date = getFormattedDate();
  console.log(date);
  const form = formidable({ multiples: true });
  form.parse(req, (err, fields, files) => {
    var imageFile =
      typeof files.tokenAllocationImage !== "undefined"
        ? files.tokenAllocationImage.name
        : "";
    if (imageFile != "") {
...
router.get("/edit-token-allocation", middleware_check_login, (req, res) => {
  id = req.query.id;
  tokenAllocation
    .findOne({ _id: id, deleted: "0" })
    .then(
      (tokenAllocation) => {
        res.render("admin/front-admin/edit-token-allocation", {
          err_msg,
          success_msg,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.post("/edit-token-allocation", middleware_check_login, (req, res) => {
  console.log(req.body);
  date = getFormattedDate();
  console.log(date);
  const form = formidable({ multiples: true });
...
router.get("/delete-token-allocation", (req, res) => {
  console.log("In delete token-allocation");
  id = req.query.id;
  tokenAllocation
    .findOne({ _id: id, deleted: "0" })
    .then(
      async (tokenAllocation) => {
...
router.get("/roadmap", (req, res) => {
  milestone
    ...
    .then(
      (milestoneData) => {
        res.render("admin/front-admin/roadmap", {
          err_msg,
          success_msg,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.get("/add-new-milestone", (req, res) => {
  err_msg = req.flash("err_msg");
  success_msg = req.flash("success_msg");

  res.render("admin/front-admin/add-new-milestone", {
    err_msg,
    success_msg,
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
...
router.post("/add-new-milestone", middleware_check_login, async (req, res) => {
  console.log(req.body);
...
router.get("/edit-milestone", (req, res) => {
  id = req.query.id;
  milestone
    .findOne({ _id: id, deleted: "0" })
    .then(
      (milestone) => {
        res.render("admin/front-admin/edit-milestone", {
          err_msg,
          success_msg,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.post("/edit-milestone", middleware_check_login, (req, res) => {
...
router.get("/delete-milestone", (req, res) => {
  console.log("In edit milestone");
  id = req.query.id;
  milestone
    .findOne({ _id: id, deleted: "0" })
...
router.get("/whitepaper", (req, res) => {
  whitepaperInfo
    ...
    .then(
      (whitePaperData) => {
        res.render("admin/front-admin/whitepaper", {
          err_msg,
          success_msg,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.get("/add-new-whitepaper", middleware_check_login, (req, res) => {
  res.render("admin/front-admin/add-new-whitepaper", {
    err_msg,
    success_msg,
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
...
router.post("/add-new-whitepaper", middleware_check_login, (req, res) => {
  console.log(req.body);
  date = getFormattedDate();
  console.log(date);
  const form = formidable({ multiples: true });
  form.parse(req, (err, fields, files) => {
    var imageFile1 =
      typeof files.whitepaper !== "undefined" ? files.whitepaper.name : "";
    if (imageFile1 != "") {
    ...
    var imageFile2 =
      typeof files.pitchDeck !== "undefined" ? files.pitchDeck.name : "";
    if (imageFile2 != "") {
...
router.get("/edit-whitepaper", middleware_check_login, (req, res) => {
  id = req.query.id;
  ...
  whitepaperInfo
    ...
    .then(
      (whitepaper) => {
        res.render("admin/front-admin/edit-whitepaper", {
          err_msg,
          success_msg,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.post("/edit-whitepaper", middleware_check_login, (req, res) => {
  //console.log(req.body)
  date = getFormattedDate();
  console.log(date);
  const form = formidable({ multiples: true });
...
router.get("/delete-whitepaper", (req, res) => {
...
router.get("/blog-list", async (req, res) => {
  ...
  res.render("admin/front-admin/blog-list", {
    err_msg,
    success_msg,
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
...
router.get("/add-new-blog", (req, res) => {
  res.render("admin/front-admin/add-new-blog", {
    err_msg,
    success_msg,
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
...
router.post("/add-new-blog", middleware_check_login, (req, res) => {
  console.log(req.body);
  date = getFormattedDate();
  console.log(date);
  const form = formidable({ multiples: true });
  form.parse(req, (err, fields, files) => {
    var imageFile =
      typeof files.blogImage !== "undefined" ? files.blogImage.name : "";
    if (imageFile != "") {
      blogImage = imageFile;
      if (imageFile != "") {
...
router.get("/edit-blog", (req, res) => {
  ...
  blogInfo
    ...
    .then(
      (blog) => {
        res.render("admin/front-admin/edit-blog", {
          err_msg,
          success_msg,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.post("/edit-blog", middleware_check_login, (req, res) => {
  ...
  form.parse(req, (err, fields, files) => {
    if (files != null && files != undefined && files != "") {
      var imageFile =
        typeof files.blog_image !== "undefined" ? files.blog_image.name : "";
    } else {
      var imageFile = "";
    }
    if (imageFile != "") {
...
router.get("/add-new-question", (req, res) => {
...
router.post("/add-new-question", middleware_check_login, (req, res) => {
  const form = formidable({ multiples: true });
  form.parse(req, (err, fields, files) => {
    var imageFile =
      typeof files.banner_image !== "undefined" ? files.banner_image.name : "";
...
router.get("/edit-question", middleware_check_login, (req, res) => {
  let id = req.query.id;
  FAQ.findOne({ _id: id, deleted: "0" })
    .then(
      (question) => {
...
router.post("/edit-question", middleware_check_login, (req, res) => {
  const form = formidable({ multiples: true });
  form.parse(req, (err, fields, files) => {
    var question = fields.question;
    var answer = fields.editor1;
...
router.get("/delete-question", middleware_check_login, (req, res) => {
...
/////////////************* terms-n-conditions ************//////////////
router.get("/terms-n-conditons", middleware_check_login, (req, res) => {
  termsAndConditionInfo
    .find({ deleted: "0" })
    .then(
      (termsAndConditionDetails) => {
        res.render("admin/front-admin/terms-n-conditons", {
          err_msg,
          success_msg,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.get("/add-terms-n-conditons", middleware_check_login, (req, res) => {
  res.render("admin/front-admin/add-terms-n-conditons", {
    err_msg,
    success_msg,
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
...
router.post("/add-terms-n-conditons", middleware_check_login, (req, res) => {
  const form = formidable({ multiples: true });
  form.parse(req, (err, fields, files) => {
    var title = fields.title;
    var content = fields.editor1;
...
router.get("/edit-terms-n-conditons", middleware_check_login, (req, res) => {
  let id = req.query.id;
  termsAndConditionInfo
    .findOne({ _id: id, deleted: "0" })
    .then(
      (termsAndCondition) => {
        res.render("admin/front-admin/edit-terms-n-conditons", {
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
          session: req.session,
          expressFlash: req.flash(),
...
router.post("/edit-terms-n-conditons", middleware_check_login, (req, res) => {
  const form = formidable({ multiples: true });
  form.parse(req, (err, fields, files) => {
    var title = fields.title;
    var content = fields.editor1;
    ...
    //console.log(fields)
    id = req.query.id;
    termsAndConditionInfo.updateOne(
      { _id: id },
      {
        $set: {
          title: title,
...
router.get("/delete-terms-n-conditons", middleware_check_login, (req, res) => {
  console.log("In delete terms-n-conditons");
  id = req.query.id;
  termsAndConditionInfo
    .findOne({ _id: id, deleted: "0" })
    .then(async (termsAndCondition) => {
...
////////////**************  Privacy-Policy ************///////////////
router.get("/privacy-policy", middleware_check_login, (req, res) => {
  privacyPolicyInfo
    .find({ deleted: "0" })
    .then(
      (privacyPolicyDetais) => {
        res.render("admin/front-admin/privacy-policy", {
          err_msg,
          success_msg,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.get("/add-admin-privacy-policy", middleware_check_login, (req, res) => {
  res.render("admin/front-admin/add-privacy-policy", {
    err_msg,
    success_msg,
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
...
router.post("/add-admin-privacy-policy", middleware_check_login, (req, res) => {
  const form = formidable({ multiples: true });
  form.parse(req, (err, fields, files) => {
    console.log(fields);
    var title = fields.title;
...
router.get("/edit-admin-privacy-policy", middleware_check_login, (req, res) => {
  let id = req.query.id;
  privacyPolicyInfo
    .findOne({ _id: id, deleted: "0" })
    .then(
      (privacyPolicy) => {
        res.render("admin/front-admin/edit-privacy-policy", {
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
          session: req.session,
          expressFlash: req.flash(),
...
////////////****************privacy policy ***************////////////////
router.get("/cookie-policy", middleware_check_login, (req, res) => {
  res.render("admin/front-admin/cookie-policy", {
    err_msg,
    success_msg,
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
...
router.get("/referral-policy", middleware_check_login, (req, res) => {
  res.render("admin/front-admin/referral-policy", {
    err_msg,
    success_msg,
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
...
router.get("/contact-list", middleware_check_login, (req, res) => {
  res.render("admin/front-admin/contact-list", {
    err_msg,
    success_msg,
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
...
router.get("/basic-details", middleware_check_login, (req, res) => {
  res.render("admin/front-admin/basic-details", {
    err_msg,
    success_msg,
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
...
router.get("/feedback-list", middleware_check_login, (req, res) => {
  res.render("admin/front-admin/feedback-list", {
    err_msg,
    success_msg,
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
...
router.get("/order-history", middleware_check_login, (req, res) => {
  err_msg = req.flash("err_msg");
  success_msg = req.flash("success_msg");
  var day = moment(new Date()).format("MM/DD/YYYY");
  // Registration.find({ deleted: '0', created_at: { $gte: day + ', 00:00:00 AM', $lte: day + ', 12:59:59 PM' } }).sort({ _id: -1 }).lean().then(async (results) => {
  ...
  OrderDetails.find({})
    .then(
      async (orderDetails) => {
        if (orderDetails) {
          res.render("admin/front-admin/order-history.ejs", {
            err_msg,
            success_msg,
            expressFlash: req.flash(),
            ...
            ebt_count: req.session.ebt_count,
            rate_per_ebt: req.session.rate_per_ebt,
            total_amnt: req.session.total_amnt,
            transaction_Id: req.session.transaction_Id,
            sender_wallet_address: req.session.sender_wallet_address,
            eth_wallet_address: req.session.eth_wallet_address,
            image: req.session.image,
            payment_type: req.session.payment_type,
...
router.get("/admin-referral-table", middleware_check_login, (req, res) => {
  err_msg = req.flash("err_msg");
  success_msg = req.flash("success_msg");

  res.render("admin/front-admin/admin-referral-table", {
    err_msg,
    success_msg,
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
...
router.get("/summary", middleware_check_login, (req, res) => {
  res.render("admin/front-admin/summary", {
    err_msg,
    success_msg,
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
...
router.get("/bonus-persent", middleware_check_login, (req, res) => {
  res.render("admin/front-admin/bonus-persent", {
    err_msg,
    success_msg,
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
...
router.get("/admin-transaction-table", middleware_check_login, (req, res) => {
  err_msg = req.flash("err_msg");
  success_msg = req.flash("success_msg");
  var day = moment(new Date()).format("MM/DD/YYYY");
  // Registration.find({ deleted: '0', created_at: { $gte: day + ', 00:00:00 AM', $lte: day + ', 12:59:59 PM' } }).sort({ _id: -1 }).lean().then(async (results) => {
  ...
  Tokendetails.find({})
    .then(
      async (tokendetails) => {
        if (tokendetails) {
          res.render("admin/front-admin/admin-transaction-table.ejs", {
            err_msg,
            success_msg,
            expressFlash: req.flash(),
            ...
            session: req.session,
            user_id: req.session.user_id,
            wallet_id: req.session.wallet_id,
            sender_wallet_address: req.session.sender_wallet_address,
            receiver_wallet_address: req.session.receiver_wallet_address,
            hash: req.session.hash,
            amount: req.session.amount,
            payment_status: req.session.payment_status,
            token_type: req.session.token_type,
            block_id: req.session.block_id,
            transaction_type: req.session.transaction_type,
            referred_to_name: req.session.referred_to_name,
            created_at: req.session.created_at,
          });
        }
      },
...
/////////////////////////////////////////////////////////////////
router.get("/site-info", middleware_check_login, (req, res) => {
  err_msg = req.flash("err_msg");
  success_msg = req.flash("success_msg");
  // /dummy insert/
  ...
  //  })
  // /dummy insert/
  GetInTouch.findOne({ deleted: "0" })
    .then(
      (Data) => {
        res.render("admin/front-admin/site-info.ejs", {
          err_msg,
          success_msg,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.post("/update_siteinfo", middleware_check_login, (req, res) => {
...
router.get("/partner-list", middleware_check_login, (req, res) => {
  err_msg = req.flash("err_msg");
  success_msg = req.flash("success_msg");

  PartnerInfo.find({ deleted: "0" })
    .then(
      (PartnerData) => {
        res.render("admin/front-admin/partner-list.ejs", {
          err_msg,
          success_msg,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.get("/add-new-partner", middleware_check_login, (req, res) => {
  res.render("admin/front-admin/add-new-partner.ejs", {
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
    session: req.session,
    expressFlash: req.flash(),
...
router.post("/add_partner", middleware_check_login, (req, res) => {
  // console.log("---------------------- ",req.body," ",req.files);
  const form = formidable({ multiples: true });
  form.parse(req, (err, fields, files) => {
    if (files != null && files != undefined && files != "") {
      var imageFile =
        typeof files.partner_image !== "undefined"
          ? files.partner_image.name
          : "";
    } else {
      var imageFile = "";
    }
...
router.get("/edit-partner", middleware_check_login, (req, res) => {
  var id = req.query.id;

  PartnerInfo.findOne({ _id: id, deleted: "0" })
    .then(
      (PartnerDetail) => {
        res.render("admin/front-admin/edit-partner.ejs", {
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
          session: req.session,
          expressFlash: req.flash(),
...
router.post("/update_partner", middleware_check_login, (req, res) => {
  // console.log("---------------------- ",req.body," ",req.files);
  const form = formidable({ multiples: true });
  form.parse(req, (err, fields, files) => {
    if (files != null && files != undefined && files != "") {
...
router.get("/delete-partner", middleware_check_login, (req, res) => {
  var current_time = Date.now();

  var id = req.query.id;

  PartnerInfo.findByIdAndUpdate(
    id,
    { $set: { deleted: "1", updated_at: current_time } },
    { new: true }
  )
...
router.get("/our-team", middleware_check_login, (req, res) => {
  err_msg = req.flash("err_msg");
  success_msg = req.flash("success_msg");
  console.log("2160 in our-team");

  TeamMember.find({ deleted: "0" })
    .then(
      (TeamData) => {
        // BannerInfo.find({deleted:'0'}).then((TeamData)=>{
        console.log(TeamData);

        res.render("admin/front-admin/our-team.ejs", {
          err_msg,
          success_msg,
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
...
router.get("/add-new-member", middleware_check_login, (req, res) => {
  res.render("admin/front-admin/add-new-member.ejs", {
    Name: req.session.user_name,
    profile_image: req.session.profile_image,
    session: req.session,
    expressFlash: req.flash(),
...
router.post("/add_member", middleware_check_login, (req, res) => {
  console.log("add member");
  const form = formidable({ multiples: true });
  form.parse(req, (err, fields, files) => {
    if (files != null && files != undefined && files != "") {
      var imageFile =
        typeof files.member_image !== "undefined"
          ? files.member_image.name
          : "";
    } else {
      var imageFile = "";
    }
...
router.get("/edit-member", middleware_check_login, (req, res) => {
  var member_id = req.query.id;

  TeamMember.findOne({ _id: member_id, deleted: "0" })
    .then(
      (MemberDetail) => {
        res.render("admin/front-admin/edit-member.ejs", {
          Name: req.session.user_name,
          profile_image: req.session.profile_image,
          session: req.session,
          expressFlash: req.flash(),
...
router.post("/update_member", middleware_check_login, (req, res) => {
  const form = formidable({ multiples: true });
  form.parse(req, (err, fields, files) => {
...
router.get("/delete-team-member", middleware_check_login, (req, res) => {
  var current_time = Date.now();
